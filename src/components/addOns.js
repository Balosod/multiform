  ...
  return (
    ...
    >
      <div className="sm:bg-white  sm:rounded-lg sm:mx-6 sm:p-6 lg:p-12">
        ...
        <div
          className={`flex flex-col sm:mt-8 md:mt-2 lg:mt-8 ${
        ...
        >
          {monthlyAddOns.map((add) => (
            <div
              key={add.addsName}
              onClick={() => handleAddsSelection(add)}
              className={`flex cursor-pointer items-center sm:p-4 md:p-1 lg:p-4 border-2 rounded-lg  mt-4 ${
                selectedAdds.some(
                  (item) =>
                    item.addsName === add.addsName && item.isMonthly === true,
                )
                  ? "border-blue-500"
                  : "border-gray-300"
              } ${addsError ? "border-red-400" : ""}`}
            >
              <input
                className="ml-1"
                id="ads-check"
              ...
              <div className="flex flex-col sm:pl-6 md:pl-2 lg:pl-6  w-full">
                ...
                <p className="sm:text-xs lg:text-sm text-gray-500">
                  {add.addsText}
                </p>
              </div>
          ...
          {yearlyAddOns.map((add) => (
            <div
              ...
              className={`flex cursor-pointer items-center sm:p-4 md:p-1 lg:p-4 border-2 rounded-lg  mt-4  ${
                selectedAdds.some(
                  (item) =>
                    item.addsName === add.addsName && item.isMonthly === false,
                )
                  ? "border-blue-500"
                  : "border-gray-300"
              } ${addsError ? "border-red-400" : ""}`}
