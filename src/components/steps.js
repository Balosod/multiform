    ...
    <div className="sm:bg-mobile md:bg-desktop md:rounded-lg lg:bg-desktop lg:rounded-md bg-no-repeat bg-cover sm:w-full md:w-2/4 lg:w-2/4 ">
      <div className="flex sm:flex-row md:flex-col  lg:flex-col  justify-center  lg:ml-0 sm:mt-6 sm:mb-16 sm:mt-6 ">
        {steps.map((step) => (
          <div
            key={step.name}
            className="flex flex-row  space-x-4 sm:pl-4 sm:pb-4 md:pb-0 md:pl-4 md:pr-6 lg:pr-0 md:pt-1 lg:pb-0 lg:pl-8 lg:pt-6"
          >
            <div
              className={`flex rounded-full mt-3 border-2 w-8 h-8 border-gray-400 mt-2 ${
                step.stage ? "bg-teal-300" : ""
              }`}
            >
              <div
                className={`mx-auto my-auto ${step.stage ? "" : "text-white"}`}
              >
                {step.figure}
              </div>
            </div>
